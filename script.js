...
async function locationSuccess(position){
  ...
  if (llreturnedData && llreturnedData.main && llreturnedData.sys) {
    let tocel = llreturnedData.main.temp - 273.15;
    temp.textContent = `Temperature: ${tocel.toFixed(2)}°C`;
    weat.textContent = `Weather: ${llreturnedData.weather[0].description.toUpperCase()}`;
    
    let country = countryCodes[llreturnedData.sys.country];
    if(llreturnedData.name.toUpperCase() === country.toUpperCase()){
      citycountry.innerText = `${country.toUpperCase()}`;
    }
    else{
...
btn.addEventListener('click', async () => {
  ...
  if (returnedData && returnedData.main && returnedData.sys) {
    ...
    if(returnedData.name.toUpperCase() === country.toUpperCase()){
      citycountry.innerText = `${country.toUpperCase()}`;
    }
    else{
